    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .center, spacing: 16) {
                CharacterImageView(image: viewModel.characterImage, isLoading: viewModel.isLoading)
                
                CharacterHeaderView(status: viewModel.detailItem.status, statusColor: viewModel.detailItem.statusColor)
        ...
        .alert(item: alertItem) { item in
            Alert(title: Text("Ошибка"), message: Text(item.id), dismissButton: .default(Text("OK")))
        }
    }
