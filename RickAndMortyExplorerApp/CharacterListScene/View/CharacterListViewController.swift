...
final class CharacterListViewController: UIViewController {
    ...
    private func updateView() {
        characterListView.updateCharacters(toCharacterItems(state.characters), isLoading: state.isLoading)
    }
    ...
    private static func createCharacterListItem(from character: Character) -> CharacterListItem {
    ...
    private func handleCharacterSelection(_ characterItem: CharacterListItem, from cell: UITableViewCell) {
    ...
    private func loadCharacters() {
        ...
        currentTask = Task {
            ...
            do {
                ...
                router?.showAlert(title: "Ошибка сети", message: networkError.errorDescription)
            } catch {
                // Для других ошибок
                router?.showAlert(title: "Неизвестная ошибка", message: error.localizedDescription)
            }
        }
    }
}
